"""
Parametric Mixed Integer Constrained Rastrigin Problem
"""
...
class exactRastrigin(abcParamModel):
    def __init__(self, n_vars, n_integers=0):
        ...
        m.a = pe.Param(pe.RangeSet(0, n_vars-1), default=0, mutable=True)
        # variables
        m.x = {}
        # discrete
        m.x_int = pe.Var(pe.RangeSet(0, n_integers-1), bounds=(-5.12,5.12), domain=pe.Integers)
        for i in range(n_integers):
            m.x[i] = m.x_int[i]
        # continuous
        m.x_real = pe.Var(pe.RangeSet(n_integers, n_vars-1), bounds=(-5.12,5.12), domain=pe.Reals)
        for i in range(n_integers, n_vars):
            m.x[i] = m.x_real[i]
        # objective
        obj = sum(m.a[i] + m.x[i] ** 2 - m.a[i] * pe.cos(2 * pi * m.x[i]) for i in m.x)
...
if __name__ == "__main__":
    ...
    # init model
    model = exactRastrigin(n_vars=3, n_integers=2)
