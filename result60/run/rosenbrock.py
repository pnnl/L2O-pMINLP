...
def exact(loader_test, config):
    ...
    df.to_csv(f"result60/rb_exact_{num_blocks}.csv")
...
def relRnd(loader_test, config):
    ...
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    print("Number of unsolved instances: ", df["Sol"].isna().sum())
    df.to_csv(f"result60/rb_rel_{num_blocks}.csv")
...
def root(loader_test, config):
    ...
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    print("Number of unsolved instances: ", df["Sol"].isna().sum())
    df.to_csv(f"result60/rb_root_{num_blocks}.csv")
...
def rndCls(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    if penalty_growth:
        df.to_csv(f"result60/rb_cls{penalty_weight}_{num_blocks}-g.csv")
    elif config.samples == 800:
        df.to_csv(f"result60/rb_cls{penalty_weight}_{num_blocks}-s.csv")
    elif config.samples == 80000:
        df.to_csv(f"result60/rb_cls{penalty_weight}_{num_blocks}-l.csv")
    else:
        df.to_csv(f"result60/rb_cls{penalty_weight}_{num_blocks}.csv")
...
def rndThd(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    df = eval(components, model, loader_test)
    if penalty_growth:
        df.to_csv(f"result60/rb_thd{penalty_weight}_{num_blocks}-g.csv")
    elif config.samples == 800:
        df.to_csv(f"result60/rb_thd{penalty_weight}_{num_blocks}-s.csv")
    elif config.samples == 80000:
        df.to_csv(f"result60/rb_thd{penalty_weight}_{num_blocks}-l.csv")
    else:
        df.to_csv(f"result60/rb_thd{penalty_weight}_{num_blocks}.csv")
...
def lrnRnd(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    if penalty_growth:
        df.to_csv(f"result60/rb_lrn{penalty_weight}_{num_blocks}-g.csv")
    else:
        df.to_csv(f"result60/rb_lrn{penalty_weight}_{num_blocks}.csv")
...
def rndSte(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    df = eval(components, model, loader_test)
    if penalty_growth:
        df.to_csv(f"result60/rb_ste{penalty_weight}_{num_blocks}-g.csv")
    elif config.samples == 800:
        df.to_csv(f"result60/rb_ste{penalty_weight}_{num_blocks}-s.csv")
    elif config.samples == 80000:
        df.to_csv(f"result60/rb_ste{penalty_weight}_{num_blocks}-l.csv")
    else:
        df.to_csv(f"result60/rb_ste{penalty_weight}_{num_blocks}.csv")
