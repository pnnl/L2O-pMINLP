...
def exact(loader_test, config):
    ...
    df.to_csv(f"result60/nc_exact_{num_var}-{num_ineq}_new.csv")
...
def relRnd(loader_test, config):
    ...
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    print("Number of unsolved instances: ", df["Sol"].isna().sum())
    df.to_csv(f"result60/nc_rel_{num_var}-{num_ineq}_new.csv")
...
def root(loader_test, config):
    ...
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    print("Number of unsolved instances: ", df["Sol"].isna().sum())
    df.to_csv(f"result60/nc_root_{num_var}-{num_ineq}_new.csv")
...
def rndCls(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    if penalty_growth:
        df.to_csv(f"result60/nc_cls{penalty_weight}_{num_var}-{num_ineq}-g_new.csv")
    else:
        df.to_csv(f"result60/nc_cls{penalty_weight}_{num_var}-{num_ineq}_new.csv")
...
def rndThd(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    df = eval(components, model, loader_test)
    if penalty_growth:
        df.to_csv(f"result60/nc_thd{penalty_weight}_{num_var}-{num_ineq}-g_new.csv")
    else:
        df.to_csv(f"result60/nc_thd{penalty_weight}_{num_var}-{num_ineq}_new.csv")
...
def lrnRnd(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    print("Number of infeasible solutions: {}".format(np.sum(df["Num Violations"] > 0)))
    if penalty_growth:
        df.to_csv(f"result60/nc_lrn{penalty_weight}_{num_var}-{num_ineq}-g_new.csv")
    else:
        df.to_csv(f"result60/nc_lrn{penalty_weight}_{num_var}-{num_ineq}_new.csv")
...
def rndSte(loader_train, loader_test, loader_val, config, penalty_growth=False):
    ...
    df = eval(components, model, loader_test)
    if penalty_growth:
        df.to_csv(f"result60/nc_ste{penalty_weight}_{num_var}-{num_ineq}-g_new.csv")
    else:
        df.to_csv(f"result60/nc_ste{penalty_weight}_{num_var}-{num_ineq}_new.csv")
