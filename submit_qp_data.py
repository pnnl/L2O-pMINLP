...
def run_rndCls(config, num_var, num_ineq, train_size, test_size, val_size):
...
def run_rndThd(config, num_var, num_ineq, train_size, test_size, val_size):
    # parameters as input data
    num_data = train_size + test_size + val_size
    b_samples = torch.from_numpy(np.random.uniform(-1, 1, size=(num_data, num_ineq))).float()
    data = {"b":b_samples}
...
def run_rndSte(config, num_var, num_ineq, train_size, test_size, val_size):
    # parameters as input data
    num_data = train_size + test_size + val_size
    b_samples = torch.from_numpy(np.random.uniform(-1, 1, size=(num_data, num_ineq))).float()
    data = {"b":b_samples}
...
for sample in [800, 8000, 80000]:
    ...
    # submit experiments
    #run_rndCls(config, num_var, num_ineq, train_size, test_size, val_size)
    #run_rndThd(config, num_var, num_ineq, train_size, test_size, val_size)
    #run_rndSte(config, num_var, num_ineq, train_size, test_size, val_size)
    print(config)
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run_rndCls, config, num_var, num_ineq, train_size, test_size, val_size)
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run_rndThd, config, num_var, num_ineq, train_size, test_size, val_size)
    print(f"Submitted job with ID: {job.job_id}")
    # STE Rounding
    executor = submitit.AutoExecutor(folder="logs")
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run_rndSte, config, num_var, num_ineq, train_size, test_size, val_size)
    print(f"Submitted job with ID: {job.job_id}")
