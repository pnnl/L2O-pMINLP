...
import run
print("Linear")
#run.linear.exact(loader_test, config)
#run.linear.relRnd(loader_test, config)
#run.linear.root(loader_test, config)
#run.linear.rndCls(loader_train, loader_test, loader_val, config)
#run.linear.rndThd(loader_train, loader_test, loader_val, config)
#run.linear.lrnRnd(loader_train, loader_test, loader_val, config)
#run.linear.rndSte(loader_train, loader_test, loader_val, config)
if config.size <= 20:
    # exact solver
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.linear.exact, loader_test, config)
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.linear.relRnd, loader_test, config)
    print(f"Submitted job with ID: {job.job_id}")
    # root nodes
    executor = submitit.AutoExecutor(folder="logs")
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.linear.root, loader_test, config)
    print(f"Submitted job with ID: {job.job_id}")
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.linear.rndThd, loader_train, loader_test, loader_val, config)
    print(f"Submitted job with ID: {job.job_id}")
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.linear.rndSte, loader_train, loader_test, loader_val, config)
    print(f"Submitted job with ID: {job.job_id}")
else:
    # exact solver
    executor = submitit.AutoExecutor(folder="logs")
