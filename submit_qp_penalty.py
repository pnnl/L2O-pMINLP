...
for size in [100, 200, 500, 1000]:
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.quadratic.rndCls, loader_train, loader_test, loader_val, config, True)
    ...
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.quadratic.rndThd, loader_train, loader_test, loader_val, config, True)
    print(f"Submitted job with ID: {job.job_id}")
    # STE Rounding
    executor = submitit.AutoExecutor(folder="logs")
    executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                               ...
                               cpus_per_task=16,
                               gpus_per_node=1)
    job = executor.submit(run.quadratic.rndSte, loader_train, loader_test, loader_val, config, True)
    print(f"Submitted job with ID: {job.job_id}")
