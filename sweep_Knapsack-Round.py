...
from src.problem import msKnapsack, nmKnapsack
from src.func.layer import netFC
from src.func import roundModel, roundGumbelModel, roundThresholdModel
from src.problem.neuromancer.trainer import trainer

def load_data(num_var, dim, num_data, test_size, val_size):
    """
    ...
    weights, _, c = pyepo.data.knapsack.genData(num_data=num_data, num_features=5,
    ...
    return weights, loader_train, loader_test, loader_dev
...
def train(method_config):
    ...
    """
    # number of items
    num_items = method_config.items
    # weights
    weights = method_config.weights
    # capacity
    caps = method_config.caps
...
def build_problem(config, method_config):
    ...
    func = nm.modules.blocks.MLP(insize=num_items, outsize=num_items,
    ...
    layers_rnd = netFC(input_dim=2*num_items,
                       hidden_dims=[hsize]*hlayers_rnd,
                       output_dim=num_items)
    rnd = rnd_class(layers=layers_rnd, param_keys=["c"], var_keys=["x"],
                    output_keys=["x_rnd"], bin_ind={"x":range(num_items)},
...
def eval(dataset, components, weights, caps):
    """
    ...
    model = msKnapsack(weights, caps)
    # init
    sols, objvals, conviols, elapseds = [], [], [], []
    # iterate through test dataset
    for c in tqdm(dataset.datadict["c"]):
        datapoints = {"c": torch.tensor(np.array([c]), dtype=torch.float32),
                      "name": "test"}
        # inference
        tick = time.time()
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    method_config.caps = 20    # capacity
    ...
    # load dataset
    num_var = method_config.items        # number of variables
    dim = 2                              # dimension of constraints
    num_data = 5000                      # number of data
    test_size = 1000                     # number of test size
    val_size = 1000                      # number of validation size
    weights, loader_train, loader_test, loader_dev = load_data(num_var, dim, num_data, test_size, val_size)

    # problem setting
    caps = 20
    method_config.caps = [caps] * dim
    method_config.weights = weights

    # set up project and sweep
    project_name = "Knapsack-{}-{}-{}".format(method_config.items,
                                              method_config.round,
                                              method_config.train)
