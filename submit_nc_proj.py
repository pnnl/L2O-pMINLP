...
# coding: utf-8
"""
Submit experiments for nonconvex_proj
"""
import argparse
import itertools
import random
...
#run.nonconvex_proj.rndCls(loader_train, loader_test, loader_val, config)
#run.nonconvex_proj.rndThd(loader_train, loader_test, loader_val, config)
print(config)
...
executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                           ...
                           cpus_per_task=16,
                           gpus_per_node=1)
job = executor.submit(run.nonconvex_proj.rndCls, loader_train, loader_test, loader_val, config)
...
executor.update_parameters(slurm_additional_parameters={"account": "def-khalile2",
                           ...
                           cpus_per_task=16,
                           gpus_per_node=1)
job = executor.submit(run.nonconvex_proj.rndThd, loader_train, loader_test, loader_val, config)
print(f"Submitted job with ID: {job.job_id}")
