...
class penaltyLoss(nn.Module):
    """
    Penalty loss function for linears problem
    """
    ...
    def cal_constr_viol(self, input_dict):
        ...
        rhs = b # b
        constr_violation = torch.relu(lhs - rhs).sum(dim=1) # Ax<=b
        # bounds
        lower_violation = torch.relu(- 30 - x).sum(dim=1)  # x >= -30
        upper_violation = torch.relu(x - 30).sum(dim=1)  # x <= 30
